...
<template>
    ...
    <div v-else class="w-full flex flex-col items-center gap-y-8">
        <div :class='{ "hidden": (guesses.length > 0 && guesses[guesses.length-1].full_name === athlete.full_name) || guesses.length >= 8 }' class="flex flex-col w-full justify-start items-center">
            <div class="relative w-full">
                <input type="text" v-model="name_query"
                    placeholder="Search athletes" @focus="isDropdownOpen = true" @blur="onBlur"
        ...
        <ColumnList v-if="guesses.length > 0">
            ...
            <AthleteGuess v-for="(_athlete, idx) in guesses" :key="idx" :_athlete="_athlete" :athlete="athlete" />
        </ColumnList>
