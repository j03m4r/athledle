...
function selectAthlete(key: string) {
    ...
    const _athlete = athletes.value.filter((item) => key===`${item.full_name}${item.team}${item.dob}`)[0]
    ...
    <div v-else class="w-full flex flex-col items-center gap-y-8">
        <div v-if="!((guesses.length > 0 && athlete && guesses[guesses.length - 1].full_name === athlete.full_name) || guesses.length >= 8)"
            ...
            <div class="relative w-full">
                <input ref="input" type="text" v-model="name_query" placeholder="Search athletes" @focus="isDropdownOpen = true" @blur="onBlur" @keydown.enter="selectAthlete(0)"
                ...
                <font-awesome-icon @click="onClearInput()" icon="x" :class='{ "hidden": name_query==="" }' class="cursor-pointer text-sm absolute right-4 top-6 text-gray-400" />
                <div v-if="isDropdownOpen"
                  ...
                  <div v-for="(athlete, idx) in athletes.filter((athlete) => !guesses_names.includes(athlete.full_name))" :key="idx" @click="selectAthlete(`${athlete.full_name}${athlete.team}${athlete.dob}`)" class="flex gap-x-2 px-8 py-4 border border-white
